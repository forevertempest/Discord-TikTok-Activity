...
const Feed = ({ user, startVideoId, onStarted, onOpenProfile }) => {
  ...
  useEffect(() => {
    ...
    async function fetchVideos() {
      try {
        const res = await axios.get('/api/videos/trending', { params: { limit: 100 } });
        if (!mounted) return;
  ...
  useEffect(() => {
    if (!startVideoId || videos.length === 0) return;

    const index = videos.findIndex((item) => item.id === startVideoId);
    if (index >= 0) {
      setTimeout(() => goToVideo(index), 40);
    }
    onStarted?.();
  }, [startVideoId, videos, goToVideo, onStarted]);
    ...
    <>
      ...
      >
        {videos.length > 0 ? (
          ...
          <div className="state-view">
            <p>Пока нет опубликованных видео.</p>
          </div>
        )}
      </div>

      {videos.length > 1 && (
        <div className="feed-jump-controls" aria-label="Навигация по видео">
          <button
            type="button"
